...
-   Suggestions you make can be simultaneously applied to the code for \>80 papers
...
pander::pandoc.p(str_c("* ", "\"Fortunately, we also recovered a minority of knock-in larvae that had integrated the Lp293 GD construct as expected, marked with DsRedNLS only (Suppl. Fig. 1A). However, no evidence of gene drive was observed in further generations, as DsRed inheritance did not exceed 50% when crossing heterozygotes to WT.\" Do you have progeny counts for these crosses? Our database framework does not allow us to incoperate data with only percentages."))

pander::pandoc.p(str_c("* ", "\"To distinguish among these possibilities, we generated females carrying single heterozygous copies of the three unlinked transgenes SagGDzpg, Lp::Sc2A10 and a non fluorescent (puromycin resistance-marked) vasa-Cas9.\" To account for possible deposition, could you let us know what cross was performed to create these triple-hets (e.g., female Lp-210; Saglin-GD X male vasa-Cas9)?"))
