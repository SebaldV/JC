...
```{r , eval="Pescod2023" %in% slide_pub, results = 'asis'}

pander::pandoc.header(str_c(clarif_text, "Pescod2023"), level = 2)

pander::pandoc.p(str_c("* ", "Could you provide the raw progeny counts associated with the inheritance bias crosses in S1/Fig4 and Fig5."))

pander::pandoc.p(str_c("* ", "\"Mosquitoes from the vas2-5958 colony were backcrossed to G3 and offspring were screened for the gene drive marker; 65 F1 males were mated individually to three G3 females, with eggs collected from each group and screened for the gene drive element.\" \n To account for possible deposition, could you let us know what F0 cross was performed to create the F1 hets? (e.g., female homozygous for vas2-5958 to WT males)"))
```
